...
class PrestaShopWebservice
{
	...
	const psCompatibleVersionsMax = '1.6.2.14';
	...
	*/
	...
	protected function checkStatusCode($status_code, $body='')
	{
		...
		switch($status_code)
		{
			...
			case 404: throw new PrestaShopWebserviceException(sprintf($error_label, $status_code, 'Not Found'), $status_code);break;
			case 405: throw new PrestaShopWebserviceException(sprintf($error_label, $status_code, 'Method Not Allowed'), $status_code);break;
			case 500: throw new PrestaShopWebserviceException(sprintf($error_label, $status_code, 'Internal Server Error'), $status_code);break;
			default: throw new PrestaShopWebserviceException('This call to PrestaShop Web Services returned an unexpected HTTP status of:' . $status_code, $status_code);
		}
	}
	/**
	...
	public function add($options)
	{
		...
		if (isset($options['resource'], $options['postXml']) || isset($options['url'], $options['postXml'])) {
			...
			$post_data = $options['postXml'];
		} else if ($options['resource']=='images' && isset($options['image'], $options['type'], $options['id'])) {
			if (!file_exists($options['image']))
				throw new PrestaShopWebserviceException('File not found');
		...
		// Add the shop identifiers if set
	...
	public function get($options)
	{
		...
		elseif (isset($options['resource']))
		{
			...
			$params = array('date', 'filter', 'display', 'sort', 'limit', 'id_shop', 'id_group_shop', 'schema', 'query', 'language');
			foreach ($params as $p)
